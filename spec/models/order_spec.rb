...
describe Order do
  let(:order) { FactoryGirl.create(:order, user_id: 1) }
  subject { order }
  ...
  describe "#set_total!" do
    before(:each) do
      ...
      @order = FactoryGirl.build(:order, product_ids: [product_2.id, product_1.id])
    end
  ...
  describe "#valid?" do
    ...
    it "becomes invalid due to insufficient product quantity" do
